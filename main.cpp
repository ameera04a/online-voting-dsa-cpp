...
public:
  ...
  void display() {
    ...
    cout << YELLOW << "Candidate         Party               Votes" << RESET
         << endl;
    cout << "--------------------------------------------" << endl;
  ...
  void printCandidates(Candidate *node) {
    ...
    cout << node->name;
    for (int i = node->name.length(); i < 18; i++)
      cout << " ";

    cout << node->party;
    for (int i = node->party.length(); i < 20; i++)
      cout << " ";
  ...
  Candidate *findCandidate(string name) { return search(root, name); }
...
void loadCitizensFromFile(string citizens) {
  ...
  while (getline(fin, cnic, ',')) {
    ...
    if (password == "-") {
      ...
      if (votedFor == "-" || votedFor == "-\r" || votedFor == " -") {
        c.votedFor = "";
      } else {
        c.votedFor = votedFor;
      }
...
void adminPanel() {
  ...
  do {
    ...
    case 3: {
      ...
      if (maxCand)
        cout << YELLOW << ">> Leading: " << maxCand->name << " ("
             << maxCand->party << ") with " << maxCand->votes << " votes"
             << endl
             << RESET;
...
bool login(string &loggedCNIC) {
  for (int attempts = 0; attempts < 3; attempts++) {
    ...
    if (usersMap.find(loggedCNIC) == usersMap.end()) {
      ...
      if (usersMap[loggedCNIC].password.empty()) {
        cout << YELLOW<<"First-time login. Please set a password: ";
        string pw;
        cin >> pw;
  ...
  cout << RED << "Too many failed attempts. Back to main menu." << RESET
       << endl;
  return false;
}
    ...
    if (choice == 1) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        if (adminPass == "admin123") {
          ...
          cout << GREEN << "\nLogged in as Admin" << endl << RESET;
          adminPanel();
          break;
        } else {
          cout << RED << "\nWrong Password. Access Denied." << endl << RESET;
        }
      }
      if (i == 3) {
      ...
      if (login(userCNIC)) {
        ...
        if (c.age < 18) {
          cout << RED << "\nYou are under 18. You can only view results.\n" << endl
               << RESET;
          electionTree.display();
        } else {
          voterPanel(userCNIC);
      ...
      cout << RED << "\nInvalid choice. Please try again." << endl << RESET;
    }
  } while (choice != 3);
